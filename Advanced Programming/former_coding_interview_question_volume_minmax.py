'''Former Coding Interview Question: Volume
...
def main():
    ...
    for i in range(1, len(bar_heights) - 1):
        ...
        leftmax = bar_heights[i]
        for j in range(i):
            leftmax = max(leftmax, bar_heights[j])

        # Find the maximum element on its right
        rightmax = bar_heights[i]
        for j in range(i + 1, len(bar_heights)):
            rightmax = max(rightmax, bar_heights[j])

        # Update the capacity of water
        capacity += (min(leftmax, rightmax) - bar_heights[i])
